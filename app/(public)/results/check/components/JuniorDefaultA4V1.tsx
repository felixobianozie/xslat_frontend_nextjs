...
// ─────────────────────────────────────────────────────────────────────────────
// JuniorDefaultA4V1.tsx
//
// Registered against the backend template_key `junior-default-a4v1` (DB
// name: "JUNIOR DEFAULT A4V1"). Assigned to arms whose result_template
// FK points at that row on the backend's ResultTemplate table.
//
// Design language — indigo accent, A4 portrait, grade summary in the
// cognitive table footer, per-student position surfaced in the identity
// grid — faithfully ported from terminal-report-template-1_junior-secondary.html.
//
// Data mapping (from the merged StudentResultResponse consumed by
// resolveTemplate on the frontend):
...
//   Teacher ID col.   ← result.subjects[*].teachers[0].public_id (merged in
//                       from the /academics/student/result/context/ endpoint).
...
//   - School logo image: served via next/image from /images/lhs_logo.png;
//     a placeholder circle showing the school abbreviation renders when
//     the image is absent or fails to load (see LogoWithFallback below).
...
// Fixed "Date Issued" value stamped on every result sheet.
//
// Kept as a static string rather than `new Date()` so the date doesn't
// drift when a parent reopens their child's report a week or a month
// later — the value should represent when the results were officially
// issued by the school, not when the sheet is being viewed.
//
// TO UPDATE: change this constant when a new batch of results is issued.
// Same treatment across all templates.
const DATE_OF_ISSUE = "24 Jul 2026";
...
function JuniorDefaultA4V1({ result, printRef }: ResultTemplateProps) {
  const {
    student_first_name,
    student_middle_name,
    student_last_name,
      ...
      >
        ...
        <section className="mt-3">
          ...
          <div className="grid grid-cols-5 gap-0 overflow-hidden rounded-md border border-slate-300">
            ...
            />
            <IdentityCell label="Date Issued" value={DATE_OF_ISSUE} />
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="border border-slate-300 px-1 py-0.5 text-center text-slate-600">
                      {/* First assigned teacher's public_id, or an em-dash
                          if no teacher is assigned in the SubjectArm →
                          teachers M2M. The merge step on the frontend
                          populates `subject.teachers` from the context
                          endpoint's payload. */}
                      {subject.teachers?.[0]?.public_id ?? "—"}
                    </td>
                  </tr>
                );
              })}
        ...
        <footer className="mt-4 flex items-center justify-between border-t border-slate-300 pt-2 text-[7px] uppercase tracking-widest text-slate-500">
          <span className="normal-case tracking-normal">
            https://www.lutheranhighschool.ng/results/check
          </span>
          <span>Issued {DATE_OF_ISSUE}</span>
        </footer>
      </div>
    </>
  );
...
export default JuniorDefaultA4V1;
...
//   - Tries to load /images/lhs_logo.png via next/image so we benefit
